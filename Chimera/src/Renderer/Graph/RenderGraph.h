...
namespace Chimera {
	...
	class RenderGraph {
	public:
		RenderGraph(VulkanContext& context, ResourceManager& resourceManager, PipelineManager& pipelineManager);
		~RenderGraph();
		...
		void AddBlitPass(const std::string& renderPassName, const std::string& srcImageName, const std::string& dstImageName);
		...
		void BlitImage(VkCommandBuffer commandBuffer, std::string srcImageName, std::string dstImageName);
