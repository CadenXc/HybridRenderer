...
namespace Chimera
{
ComputeExecutionContext::ComputeExecutionContext(RenderGraph& graph,
...
{
    auto& pipe =
        PipelineManager::Get().GetComputePipeline({shaderName, shaderName});

        // --- Record Shader Name ---
    bool alreadyAdded = false;
    for (const auto& existing : m_Pass.shaderNames)
        if (existing == shaderName) alreadyAdded = true;
...
void ComputeExecutionContext::Dispatch(const std::string& shaderName,
