...
namespace Chimera
{
GBufferPass::GBufferPass(std::shared_ptr<Scene> scene) : m_Scene(scene) {}
...
void GBufferPass::Setup(PassData& data, RenderGraph::PassBuilder& builder)
{
    ...
    VkClearColorValue clearColor = {
        {frameCtx.ClearColor.r, frameCtx.ClearColor.g, frameCtx.ClearColor.b,
         frameCtx.ClearColor.a}};
    ...
    data.motion =
        builder.Write(RS::Motion)
            .Format(VK_FORMAT_R16G16B16A16_SFLOAT) // Extended to 4 channels for
                                                   // Depth + Derivative
            .Clear(clearZero)
            .SaveAsHistory(RS::Motion); // Store linear depth/gradient history
                                        // for SVGF validation
    ...
    // Write to Depth, establishing dependency on Prepass which already wrote to
    // it.
...
void GBufferPass::Execute(const PassData& data, RenderGraphRegistry& reg,
                          VkCommandBuffer cmd)
{
    if (!m_Scene) return;
    ...
    for (const auto& e : allEntities)
        if (e.mesh.model)
            totalMeshes += (uint32_t)e.mesh.model->GetMeshes().size();
