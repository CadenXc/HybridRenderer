...
namespace Chimera
{
    RTAOPass::RTAOPass(std::shared_ptr<Scene> scene)
        : m_Scene(scene)
    {
    }

    void RTAOPass::Setup(PassData& data, RenderGraph::PassBuilder& builder)
    {
        data.output = builder.WriteStorage("AORaw").Format(VK_FORMAT_R16G16B16A16_SFLOAT);
        data.normal = builder.Read(RS::Normal);
        data.depth  = builder.Read(RS::Depth);
    }

    void RTAOPass::Execute(const PassData& data, RenderGraphRegistry& reg, VkCommandBuffer cmd)
    {
        RaytracingExecutionContext ctx(reg.graph, reg.pass, cmd);
        
        RaytracingPipelineDescription desc;
        desc.raygen_shader = "RT_AO_Gen";
