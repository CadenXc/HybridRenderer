...
namespace Chimera {
    ...
    static VKAPI_ATTR VkBool32 VKAPI_CALL DebugCallback(VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity, VkDebugUtilsMessageTypeFlagsEXT messageType, const VkDebugUtilsMessengerCallbackDataEXT* pCallbackData, void* pUserData) {
        if (messageSeverity >= VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT) CH_CORE_ERROR("Validation Layer: {}", pCallbackData->pMessage);
        else if (messageSeverity >= VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT) CH_CORE_WARN("Validation Layer: {}", pCallbackData->pMessage);
        else CH_CORE_INFO("Validation Layer: {}", pCallbackData->pMessage);
        return VK_FALSE;
    }
    ...
    bool VulkanInstance::CheckValidationLayerSupport() {
        ...
        for (const char* layerName : validationLayers) {
            ...
            for (const auto& layerProps : availableLayers) {
                if (strcmp(layerName, layerProps.layerName) == 0) { found = true; break; }
            }
