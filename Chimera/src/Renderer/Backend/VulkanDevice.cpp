...
VulkanDevice::~VulkanDevice()
{
    ...
    if (m_Allocator != nullptr)
    {
        ...
        {
            CH_CORE_ERROR(
                "VulkanDevice: LEAK DETECTED! Total VMA allocations: {}, size: "
                "{} bytes",
...
void VulkanDevice::PickPhysicalDevice(VkInstance instance, VkSurfaceKHR surface)
{
    ...
    {
        candidates.insert(
            std::make_pair(RateDeviceSuitability(device, surface), device));
    }
...
void VulkanDevice::CreateLogicalDevice(VkSurfaceKHR surface)
{
    ...
    float queuePriorities[] = {
        1.0f, 1.0f}; // Request up to 2 queues if in same family
    for (uint32_t queueFamily : uniqueQueueFamilies)
    {
        ...
        // If compute and graphics are in the same family, we try to get 2
        // queues for async overlap
        uint32_t count = 0;
        ...
        if (queueFamily == indices.graphicsFamily &&
            queueFamily == indices.computeFamily &&
            families[queueFamily].queueCount > 1)
        {
            requestedCount = 2;
        }
    ...
    vkGetDeviceQueue(m_LogicalDevice, m_GraphicsQueueFamily, 0,
                     &m_GraphicsQueue);
    ...
    if (m_GraphicsQueueFamily == m_ComputeQueueFamily)
    {
        ...
        {
            vkGetDeviceQueue(m_LogicalDevice, m_ComputeQueueFamily, 1,
                             &m_ComputeQueue);
        }
        else
        {
    ...
    {
        vkGetDeviceQueue(m_LogicalDevice, m_ComputeQueueFamily, 0,
                         &m_ComputeQueue);
    }
...
void VulkanDevice::CreateAllocator(VkInstance instance)
{
    ...
    vmaFuncs.vkGetPhysicalDeviceMemoryProperties2KHR =
        vkGetPhysicalDeviceMemoryProperties2;
    vmaFuncs.vkGetDeviceBufferMemoryRequirements =
        vkGetDeviceBufferMemoryRequirements;
    vmaFuncs.vkGetDeviceImageMemoryRequirements =
