...
PipelineManager::~PipelineManager()
{
    ...
    CH_CORE_INFO("PipelineManager: Destroying Layout Cache ({} layouts)...",
                 m_LayoutCache.size());
    ...
    CH_CORE_INFO("PipelineManager: Destroying Set2Layout Cache ({} layouts)...",
                 m_Set2LayoutCache.size());
...
void PipelineManager::ClearCache()
{
    ...
    CH_CORE_INFO("PipelineManager: Destroying Graphics Pipelines ({})...",
                 m_GraphicsCache.size());
    ...
    CH_CORE_INFO("PipelineManager: Destroying Raytracing Pipelines ({})...",
                 m_RaytracingCache.size());
    ...
    CH_CORE_INFO("PipelineManager: Destroying Compute Pipelines ({})...",
                 m_ComputeCache.size());
...
GraphicsPipeline& PipelineManager::GetGraphicsPipeline(
    ...
    const GraphicsPipelineDescription& desc)
{
    ...
    for (uint32_t i = 0; i < (uint32_t)desc.specializationConstants.size(); ++i)
    {
        specEntries.push_back(
            {i, i * (uint32_t)sizeof(uint32_t), sizeof(uint32_t)});
    }
    ...
    if (!specEntries.empty())
    {
        ...
        specInfo.dataSize =
            (uint32_t)(desc.specializationConstants.size() * sizeof(uint32_t));
    ...
    VkCompareOp compareOp = (desc.depth_compare_op != (VkCompareOp)0)
    ...
    std::vector<VkPipelineColorBlendAttachmentState> bA(
        colorFormats.size(),
    ...
    VkPipelineRenderingCreateInfo rE{
    ...
    const RaytracingPipelineDescription& desc)
{
    ...
    VkRayTracingShaderGroupCreateInfoKHR rgGroup{
    ...
    {
        uint32_t mIdx =
            addStage(ShaderManager::GetShader(desc.miss_shaders[i]).get(),
                     VK_SHADER_STAGE_MISS_BIT_KHR);
        VkRayTracingShaderGroupCreateInfoKHR mGroup{
            VK_STRUCTURE_TYPE_RAY_TRACING_SHADER_GROUP_CREATE_INFO_KHR};
    ...
    {
        VkRayTracingShaderGroupCreateInfoKHR hGroup{
            VK_STRUCTURE_TYPE_RAY_TRACING_SHADER_GROUP_CREATE_INFO_KHR};
        ...
        {
            hGroup.closestHitShader = addStage(
                ShaderManager::GetShader(desc.hit_shaders[i].closest_hit).get(),
                VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR);
        }
        if (!desc.hit_shaders[i].any_hit.empty())
        {
            hGroup.anyHitShader = addStage(
                ShaderManager::GetShader(desc.hit_shaders[i].any_hit).get(),
    ...
    const ComputePipelineDescription::Kernel& kernel)
{
    ...
    if (!specEntries.empty())
    {
        specInfo.mapEntryCount = (uint32_t)specEntries.size();
        specInfo.pMapEntries = specEntries.data();
        specInfo.dataSize = (uint32_t)(kernel.specializationConstants.size() *
                                       sizeof(uint32_t));
...
VkPipelineLayout PipelineManager::GetReflectionLayout(
    const std::vector<const Shader*>& shaders)
{
    ...
    for (const auto* sh : shaders)
    {
        if (sh)
        {
            hash ^= std::hash<std::string>{}(sh->GetPath()) + 0x9e3779b9 +
                    (hash << 6) + (hash >> 2);
        }
    }
...
VkDescriptorSetLayout PipelineManager::GetSet2Layout(
    const std::vector<const Shader*>& shaders)
{
    ...
    {
        hash ^= std::hash<uint32_t>{}(b.binding) + 0x9e3779b9 + (hash << 6) +
                (hash >> 2);
        hash ^= std::hash<uint32_t>{}(static_cast<uint32_t>(b.type)) +
                0x9e3779b9 + (hash << 6) + (hash >> 2);
        hash ^= std::hash<uint32_t>{}(b.count) + 0x9e3779b9 + (hash << 6) +
                (hash >> 2);
    }
