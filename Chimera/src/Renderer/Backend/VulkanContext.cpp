...
namespace Chimera {
	...
	#else
	const bool enableValidationLayers = false; // Forced OFF for Win32 stability debug
	#endif
	...
	VulkanContext::VulkanContext(GLFWwindow* window)
		: m_Window(window)
	{
		CH_CORE_INFO("VulkanContext: volkInitialize...");
		if (volkInitialize() != VK_SUCCESS)
		{
			throw std::runtime_error("failed to initialize volk!");
		}

		CH_CORE_INFO("VulkanContext: CreateInstance...");
		CreateInstance();
		
		CH_CORE_INFO("VulkanContext: volkLoadInstance...");
		volkLoadInstance(m_Instance);

		CH_CORE_INFO("VulkanContext: SetupDebugMessenger...");
		SetupDebugMessenger();
		
		CH_CORE_INFO("VulkanContext: CreateSurface...");
		CreateSurface();
		
		CH_CORE_INFO("VulkanContext: PickPhysicalDevice...");
		PickPhysicalDevice();
		
		CH_CORE_INFO("VulkanContext: CreateLogicalDevice...");
		CreateLogicalDevice();
		
		CH_CORE_INFO("VulkanContext: volkLoadDevice...");
		volkLoadDevice(m_Device);

		CH_CORE_INFO("VulkanContext: CreateAllocator...");
		CreateAllocator();
		
		CH_CORE_INFO("VulkanContext: CreateCommandPool...");
		CreateCommandPool();

		CH_CORE_INFO("VulkanContext: CreateSwapChain...");
		CreateSwapChain();
		
		CH_CORE_INFO("VulkanContext: CreateImageViews...");
		CreateImageViews();
	...
	void VulkanContext::PickPhysicalDevice()
	{
		...
		// Use a multimap to automatically sort candidates by score
	...
	void VulkanContext::CreateLogicalDevice()
	{
		...
		scalarBlockLayoutFeatures.pNext = &rayTracingPipelineFeatures;
	...
	bool VulkanContext::CheckDeviceExtensionSupport(VkPhysicalDevice device)
	{
		...
		for (const char* ext : deviceExtensions) requiredExtensions.insert(ext);
