...
namespace Chimera
{
        ...
        : m_Width(width), m_Height(height), m_Format(format), m_MipLevels(mipLevels)
    {
        ...
        if (vkCreateImageView(VulkanContext::Get().GetDevice(), &viewInfo, nullptr, &m_View) != VK_SUCCESS)
        {
    ...
    Image::~Image()
    {
        ...
        {
            vmaDestroyImage(VulkanContext::Get().GetAllocator(), m_Image, m_Allocation);
        }
    }
