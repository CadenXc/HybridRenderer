...
Image::Image(uint32_t width, uint32_t height, VkFormat format,
             ...
             VkImageTiling tiling, const std::string& name)
    : m_Width(width), m_Height(height), m_Format(format), m_MipLevels(mipLevels)
{
    ...
    CH_CORE_TRACE("Image: ALLOCATED. Handle: [0x{:x}], Size: {}x{}, Name: {}",
                  (uint64_t)m_Image, m_Width, m_Height, name);

    if (!name.empty())
    {
        VulkanContext::Get().SetDebugName((uint64_t)m_Image,
    ...
    if (!name.empty())
    {
        ...
        VulkanContext::Get().SetDebugName(
            (uint64_t)m_View, VK_OBJECT_TYPE_IMAGE_VIEW, viewName.c_str());
    }
}
...
Image::~Image()
{
    ...
    {
        CH_CORE_TRACE("Image: DESTROYING. Handle: [0x{:x}], Size: {}x{}",
                      (uint64_t)m_Image, m_Width, m_Height);
