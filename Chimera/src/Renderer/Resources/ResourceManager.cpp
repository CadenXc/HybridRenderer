...
ResourceManager::ResourceManager()
{
    ...
    CH_CORE_INFO("ResourceManager: Initialized. m_Context count: {}",
                 m_Context.use_count());
}
...
void ResourceManager::Clear()
{
    ...
    {
        CH_CORE_INFO(
            "ResourceManager: Clear() skipped (m_IsCleared: {}, m_Context: {})",
            m_IsCleared, (bool)m_Context);
        return;
    }
    ...
    if (m_ActiveScene)
    {
        ...
        // 2. Clear Asset Maps
    CH_CORE_INFO("ResourceManager: Clearing {} Materials...",
                 m_Materials.size());
    m_Materials.clear();
    m_MaterialMap.clear();
    ...
    for (size_t i = 0; i < m_Textures.size(); ++i)
    {
        if (m_Textures[i])
        {
            CH_CORE_INFO("ResourceManager: Destroying texture [ID: {}]",
                         (uint32_t)i);
            m_Textures[i].reset();
        }
    }
    ...
    CH_CORE_INFO("ResourceManager: Clearing {} Uniform Buffers...",
...
void ResourceManager::InitGlobalResources()
{
    ...
    m_PrimitiveBuffer = std::make_unique<Buffer>(
        sizeof(GpuPrimitive) * 4096,
        VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
...
void ResourceManager::ResetTransientDescriptorPool()
{
    VkDescriptorPool currentPool =
        m_TransientDescriptorPools[m_CurrentFrameIndex];
...
void ResourceManager::CreateTransientDescriptorPools()
{
    ...
    for (uint32_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
    {
        vkCreateDescriptorPool(m_Context->GetDevice(), &poolInfo, nullptr,
                               &m_TransientDescriptorPools[i]);
    }
}
...
void ResourceManager::CreateTextureSampler()
{
    ...
    vkCreateSampler(m_Context->GetDevice(), &samplerInfo, nullptr,
                    &m_TextureSampler);
}
...
void ResourceManager::CreateSceneDescriptorSetLayout()
{
    std::vector<VkDescriptorSetLayoutBinding> b = {
        ...
        {BINDING_MATERIALS, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1,
         VK_SHADER_STAGE_ALL, nullptr},
        {BINDING_PRIMITIVES, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1,
         VK_SHADER_STAGE_ALL, nullptr},
...
void ResourceManager::UpdateSceneDescriptorSet(Scene* s, uint32_t fI)
{
    ...
    for (uint32_t i = start; i < end; ++i)
    {
        ...
        if (m_PrimitiveBuffer &&
            m_PrimitiveBuffer->GetBuffer() != VK_NULL_HANDLE)
        {
    ...
    const std::string& path, std::shared_ptr<Scene> targetScene)
{
    ...
    CH_CORE_INFO("ResourceManager: Requesting FULL ASYNC load for model: {0}",
                 path);
    ...
    auto future = Application::Get().GetTaskSystem()->Enqueue(
        [path, model]() -> std::shared_ptr<ImportedScene>
        {
            ...
            {
                CH_CORE_INFO(
                    "ResourceManager: Background thread starting GPU upload "
                    "for {0}...",
                    path);
...
void ResourceManager::UpdateLoadingTasks()
{
    for (auto it = m_LoadingModels.begin(); it != m_LoadingModels.end();)
    {
            ...
            std::future_status::ready)
        {
            ...
            {
                // If a target scene was provided, integrate the nodes and
                // materials NOW
                if (task.targetScene)
                {
                    CH_CORE_INFO(
                        "ResourceManager: Integrating fully ready model {0} "
                        "into scene...",
                        task.path);
            ...
            {
                CH_CORE_ERROR("ResourceManager: Failed to load model {0}",
                              task.path);
            }
...
void ResourceManager::SyncPrimitivesToGPU(Scene* scene)
{
    ...
    if (m_PrimitiveBuffer->GetSize() < dataSize)
    {
        m_PrimitiveBuffer = std::make_unique<Buffer>(
            ...
            VMA_MEMORY_USAGE_GPU_ONLY, "Global_PrimitiveBuffer_Resized");
    }

    Buffer staging(dataSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                   VMA_MEMORY_USAGE_CPU_ONLY, "Staging_SyncPrimitives");
...

    {
        ...
        vkCmdCopyBuffer(cmd, (VkBuffer)staging.GetBuffer(),
                        (VkBuffer)m_PrimitiveBuffer->GetBuffer(), 1, &copy);
    }
}
...
void ResourceManager::DestroyGraphImage(GraphImage& i)
{
    ...
    VkDevice device = m_Context->GetDevice();
    CH_CORE_INFO("ResourceManager: Destroying GraphImage [0x{:x}].",
                 (uint64_t)i.handle);
                                          ...
                                          const std::string& n)
{
    ...
    CH_CORE_INFO("ResourceManager: Registered Texture [ID: {}, Name: {}].", idx,
                 n);
    m_Textures.push_back(std::move(t));
                                            ...
                                            const std::string& n)
{
    ...
    CH_CORE_INFO("ResourceManager: Registered Material [ID: {}, Name: {}].",
                 idx, n);
...
void ResourceManager::Release(MaterialHandle h)
{
    ...
    {
        CH_CORE_INFO(
            "ResourceManager: Material [ID: {}] reference count zero. "
            "Resetting to default.",
            h.id);

        // [FIX] Reset the material content so a new model reusing this ID won't
        // inherit old properties
        if (m_Materials[h.id])
        {
            m_Materials[h.id]->SetData(
                GpuMaterial{}); // Clear to standard PBR defaults
        }
