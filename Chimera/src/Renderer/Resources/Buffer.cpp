...
namespace Chimera
{
        ...
        : m_Size(size)
    {
        ...
        CH_CORE_TRACE("Buffer: ALLOCATED. Handle: [0x{:x}], Size: {}, Name: {}", (uint64_t)m_Buffer, m_Size, name);
    ...
    Buffer::~Buffer()
    {
        ...
        {
            CH_CORE_TRACE("Buffer: DESTROYING. Handle: [0x{:x}], Size: {}", (uint64_t)m_Buffer, m_Size);
            if (m_DeviceAddress != 0)
            {
                CH_CORE_TRACE("Buffer: Releasing Device Address [0x{:x}]", m_DeviceAddress);
            }
