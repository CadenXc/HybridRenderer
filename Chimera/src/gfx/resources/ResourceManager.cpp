...
namespace Chimera {
	...
	void ResourceManager::CreateDescriptorSets()
	{
		...
		for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
			...
			imageInfo.imageView = m_GlobalTexture->GetImageView();
	...
	VulkanUtils::TransitionImageLayout(m_Context, image->GetImage(), format, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1);
	...
	VulkanUtils::TransitionImageLayout(m_Context, image->GetImage(), format, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, 1);
