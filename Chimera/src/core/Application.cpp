    ...
    : m_Specification(spec)
{
    ...
    for (int i = 0; i < 5; ++i)
    {
        if (std::filesystem::exists(root / "Chimera") &&
            std::filesystem::exists(root / "scripts"))
        {
            foundRoot = true;
            break;
        }
    ...
    if (foundRoot)
    {
        ...
        {
            m_Specification.ShaderSourceDir =
                (root / "Chimera" / "shaders").string();
            CH_CORE_INFO("Application: Detected Shader Source Dir: {0}",
                         m_Specification.ShaderSourceDir);
        }
    }
    ...
    CH_CORE_INFO(
        "Application: Initialized with latest SVGF ShaderRegistry mappings.");
...
{
    CH_CORE_INFO(
        "Application: Destructor started. Starting surgical teardown...");
    ...
    if (m_Context)
    {
        try
        {
            ...
            while (m_LayerStack.size() > 1)
            {
                ...
                {
                    CH_CORE_INFO("Application: Releasing layer: {}",
                                 layer->GetName());
                    layer->OnDetach();
            ...
            // 6. [STEP 3] DESTROY CORE RENDER SUBSYSTEMS (This releases the 6
            // leaking buffers)
            CH_CORE_INFO(
            ...
            {
                CH_CORE_INFO(
                    "Application: [Step 4/5] Clearing ResourceManager pool...");
                ...
                CH_CORE_INFO(
                    "Application: Calling m_ResourceManager->Clear()...");
                m_ResourceManager->Clear();
                CH_CORE_INFO(
                    "Application: m_ResourceManager->Clear() completed.");

                CH_CORE_INFO(
                    "Application: Calling m_ResourceManager.reset()...");
                m_ResourceManager.reset();
                CH_CORE_INFO(
                    "Application: m_ResourceManager.reset() completed.");
            }
        ...
        {
            CH_CORE_ERROR(
                "Application: CRITICAL CRASH during destruction sequence! "
    ...
    CH_CORE_INFO(
        "Application: Teardown finished successfully. Context count: {}",
        contextKeepAlive.use_count());
    s_Instance = nullptr;
}
...
void Application::UpdateGlobalUBO(uint32_t frameIndex)
{
    ...
    ubo.camera.viewProjInverse =
        glm::inverse(ubo.camera.proj * ubo.camera.view);
    ...
    ubo.camera.jitterData =
        glm::vec4(m_FrameContext.Jitter.x, m_FrameContext.Jitter.y,
                  m_FrameContext.PrevJitter.x, m_FrameContext.PrevJitter.y);
    ...
    if (m_ResourceManager->HasActiveScene())
    {
        ...
        ubo.sunLight.intensity =
            glm::vec4(sceneLight.color.a, sceneLight.direction.w, 0.0f, 0.0f);
    }
    else
    {
        ubo.sunLight.direction =
            glm::vec4(glm::normalize(glm::vec3(-1.0f, -1.0f, -1.0f)), 0.0f);
    ...
    // Block 1: displayData
    ubo.displayData =
        glm::vec4(m_FrameContext.ViewportSize.x, m_FrameContext.ViewportSize.y,
                  1.0f / m_FrameContext.ViewportSize.x,
    ...
    {
        if (m_RenderPath &&
            m_RenderPath->HasUsableHistory("TAAOutput"))
        {
    ...
    // Block 3: postData
    ubo.postData =
        glm::vec4(m_FrameContext.Exposure, m_FrameContext.AmbientStrength, 0.0f,
                  (float)m_BlueNoiseTextureIndex);
    ...
    ubo.svgfAlpha =
        glm::vec4(0.01f, 0.1f, 0.0f,
                  0.0f); // Tighter Alpha: Color=0.01 (1%), Moments=0.1 (10%)
    ubo.svgfPhi = glm::vec4(
        4.0f, 128.0f, 0.02f,
        0.0f); // Tightened: PhiColor=4.0, PhiNormal=128.0, PhiDepth=0.02
