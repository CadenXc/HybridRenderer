...
namespace Chimera {
	...
	void ImGuiLayer::OnAttach()
	{
		...
		ImGui_ImplGlfw_InitForVulkan(m_Context->GetWindow(), false); // Change true to false
	...
	void ImGuiLayer::OnEvent(Event& e)
	{
		...
		dispatcher.Dispatch<MouseButtonPressedEvent>([this](MouseButtonPressedEvent& e) {
			ImGui_ImplGlfw_MouseButtonCallback(m_Context->GetWindow(), (int)e.GetMouseButton(), GLFW_PRESS, 0);
			return false;
		});
		dispatcher.Dispatch<MouseButtonReleasedEvent>([this](MouseButtonReleasedEvent& e) {
			ImGui_ImplGlfw_MouseButtonCallback(m_Context->GetWindow(), (int)e.GetMouseButton(), GLFW_RELEASE, 0);
			return false;
		});
		dispatcher.Dispatch<MouseMovedEvent>([this](MouseMovedEvent& e) {
			ImGui_ImplGlfw_CursorPosCallback(m_Context->GetWindow(), e.GetX(), e.GetY());
			return false;
		});
		dispatcher.Dispatch<MouseScrolledEvent>([this](MouseScrolledEvent& e) {
			ImGui_ImplGlfw_ScrollCallback(m_Context->GetWindow(), e.GetXOffset(), e.GetYOffset());
			return false;
		});
		dispatcher.Dispatch<KeyPressedEvent>([this](KeyPressedEvent& e) {
			ImGui_ImplGlfw_KeyCallback(m_Context->GetWindow(), (int)e.GetKeyCode(), 0, GLFW_PRESS, 0);
			return false;
		});
		dispatcher.Dispatch<KeyReleasedEvent>([this](KeyReleasedEvent& e) {
			ImGui_ImplGlfw_KeyCallback(m_Context->GetWindow(), (int)e.GetKeyCode(), 0, GLFW_RELEASE, 0);
			return false;
		});
		dispatcher.Dispatch<KeyTypedEvent>([this](KeyTypedEvent& e) {
			ImGui_ImplGlfw_CharCallback(m_Context->GetWindow(), (unsigned int)e.GetKeyCode());
			return false;
		});
