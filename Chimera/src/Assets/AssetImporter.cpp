...
namespace Chimera {
    ...
    std::shared_ptr<ImportedScene> AssetImporter::ImportScene(const std::string& path, ResourceManager* resourceManager) {
        ...
        if (result != cgltf_result_success) { cgltf_free(data); return nullptr; }
        ...
        auto LoadTex = [&](cgltf_texture* tex) {
            if (!tex || !tex->image) return TextureHandle();
            if (textureCache.count(tex->image)) return textureCache[tex->image];
            
            std::string baseDir = path.substr(0, path.find_last_of("/\\") + 1);
            std::string texPath = baseDir + tex->image->uri;
            TextureHandle idx = resourceManager->LoadTexture(texPath);
            textureCache[tex->image] = idx;
            return idx;
        };
