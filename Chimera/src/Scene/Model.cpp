...
void Model::UploadToGPU(const ImportedScene& importedScene)
{
    ...
    if (triangleCount > 0)
    {
        stagingTBO = std::make_unique<Buffer>(triangleBufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_CPU_ONLY);
    ...
    m_IndexBuffer = std::make_unique<Buffer>(indexBufferSize,
    ...
    if (triangleCount > 0)
    {
        m_TriangleBuffer = std::make_unique<Buffer>(triangleBufferSize,
    ...
    // 3. Transfer
    {
        ...
        if (triangleCount > 0)
        {
            ...
            vkCmdCopyBuffer(cmd, (VkBuffer)stagingTBO->GetBuffer(), (VkBuffer)m_TriangleBuffer->GetBuffer(), 1, &tCopy);
        }
    }
...
void Model::BuildBLAS()
{
    ...
    for (uint32_t i = 0; i < m_Meshes.size(); ++i)
    {
        ...
        VkAccelerationStructureBuildRangeInfoKHR rangeInfo{maxPrimitiveCount, 0, 0, 0};
...
Model::~Model()
{
    if (m_Context)
    {
        ...
        {
            for (auto h : m_BLASHandles) vkDestroyAccelerationStructureKHR(device, h, nullptr);
        }
        for (const auto& mesh : m_Meshes) ResourceManager::Get().Release(MaterialHandle(mesh.materialIndex));
    }
}
} // namespace Chimera
