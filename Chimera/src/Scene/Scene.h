...
namespace Chimera
{
    ...
    public:
        ...
        int GetSkyboxTextureIndex() const { return m_SkyboxRef.IsValid() ? (int)m_SkyboxRef.Get().id : -1; }
        VkBuffer GetMaterialBuffer() const { return m_ResourceManager->GetMaterialBuffer(); }
        VkBuffer GetInstanceDataBuffer() const { return m_InstanceDataBuffer ? m_InstanceDataBuffer->GetBuffer() : m_DummyBuffer->GetBuffer(); }
