...
namespace Chimera
{
    ...
    void Scene::SubdivideOctree(OctreeNode* node, uint32_t depth)
    {
        ...
        for (uint32_t idx : node->entityIndices)
        {
            ...
            for (int i = 0; i < 8; ++i)
            {
                // Simple check: if center of entity is in child, or if it intersects
                // For simplicity here: use center point
    ...
    void Scene::TraverseOctree(const OctreeNode* node, const Frustum& frustum, std::vector<uint32_t>& outVisibleIndices) const
    {
        ...
        else
        {
            ...
            {
                if (node->children[i])
                    TraverseOctree(node->children[i].get(), frustum, outVisibleIndices);
            }
        }
    }
