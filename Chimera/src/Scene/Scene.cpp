                                   ...
                                   const std::string& path)
{
    ...
    // 1. Process Materials and build local-to-global mapping
    std::unordered_map<int, uint32_t> localToGlobalMaterialMap;
    for (uint32_t i = 0; i < (uint32_t)imported->Materials.size(); ++i)
    {
        auto mat = std::make_unique<Material>();
        mat->SetData(imported->Materials[i]);
        MaterialHandle h = ResourceManager::Get().AddMaterial(std::move(mat));
        localToGlobalMaterialMap[i] = h.id;
    }

    // 2. Correctly translate mesh material indices
    for (auto& mesh : imported->Meshes)
    {
        if (localToGlobalMaterialMap.count(mesh.materialIndex))
            mesh.materialIndex = localToGlobalMaterialMap[mesh.materialIndex];
        else
            mesh.materialIndex = 0; // Default material
    }
    model->UpdateMeshes(imported->Meshes);
...
{
    if (index < m_Entities.size()) m_EntitiesToRemove.push_back(index);
}
...
void Scene::SubdivideOctree(OctreeNode* node, uint32_t depth)
{
    ...
    for (uint32_t idx : node->entityIndices)
    {
        ...
        for (int i = 0; i < 8; ++i)
        {
            ...
            const ChimeraAABB& cBounds = node->children[i]->bounds;
