...
void EditorCamera::UpdateProjection()
{
    ...
    // [FINALIZED] Always use Reversed-Z: Near maps to 1.0 (or n/(f-n) logic),
    // Far maps to 0.0
...
void EditorCamera::OnUpdate(Timestep ts, bool isHovered, bool isFocused)
{
    ...
    if (m_FocalPoint != oldFocalPoint || m_Pitch != oldPitch ||
        m_Yaw != oldYaw || m_Distance != oldDistance)
        m_IsUpdated = true;
...
void EditorCamera::OnEvent(Event& e)
{
    ...
    dispatcher.Dispatch<MouseScrolledEvent>(
        BIND_EVENT_FN(EditorCamera::OnMouseScroll));
}
