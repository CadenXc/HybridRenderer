...
layout(buffer_reference, scalar) readonly buffer Vertices { GpuVertex v[]; };
...
layout(binding = 1, set = 1, scalar) readonly buffer MaterialBuffer { GpuMaterial m[]; } materialBuffer;
...
void main() 
{
    ...
    GpuMaterial mat = materialBuffer.m[inst.materialIndex];
    ...
    if (mat.normalTex >= 0) {
        ...
        vec3 bitangent = normalize(cross(N, tangent) * (v0.tangent.w * barycentrics.x + v1.tangent.w * barycentrics.y + v2.tangent.w * barycentrics.z));
