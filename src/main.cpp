...
private:
    ...
    {
        auto app = reinterpret_cast<ChimeraApplication*>(glfwGetWindowUserPointer(window));
...
{
    Chimera::Log::Init();
    CH_CORE_INFO("Chimera Engine Initialized");

    ChimeraApplication app;
    ...
    {
        CH_CORE_FATAL("Application Crash: {}", e.what());
        std::cout << "Press Enter to exit..." << std::endl;
        std::cin.get();
        return EXIT_FAILURE;
    }

    CH_INFO("Program completed successfully. Press Enter to exit...");
    std::cin.get();
    return EXIT_SUCCESS;
}