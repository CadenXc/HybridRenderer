      ...
      m_Automation(automationOptions)
{
    ...
    m_Automation.ConfigureRenderSettings(m_RenderFlags, m_DisplayMode,
                                         m_ShowControlPanel);
...
void EditorLayer::OnUpdate(Timestep ts)
{
    ...
    Scene* scene = GetActiveSceneRaw();
    m_Automation.UpdateBeforeScene(
        scene, activePath,
        m_BenchmarkSceneState == BenchmarkSceneState::Ready,
        m_BenchmarkSceneState == BenchmarkSceneState::Failed);
    if (scene) scene->OnUpdate(ts.GetSeconds());
...
void EditorLayer::OnEvent(Event& e)
{
    ...
    if (!ImGui::GetIO().WantCaptureMouse && !benchmarkRunning &&
        !captureSequenceRunning && !m_Automation.IsActive())
    {
